  ...
  <div>
    <div>
      ...
      <el-input
        ...
        v-model="nameKeyword"
      ></el-input>
      <el-button @click="initUserData" icon="el-icon-search" type="primary"
        >搜索</el-button
      >
      <el-button @click="refreshTable" icon="el-icon-refresh" type="primary"
        >刷新表格</el-button
      >
    </div>
    <div style="margin-top: 20px;width: 1000px">
      ...
      >
        ...
        <el-table-column label="是否锁定该账户" width="200">
          <template slot-scope="scope">
            <el-switch
...
export default {
  ...
  methods: {
    ...
    changeLockedStatus(data) {
      this.putRequest(
        "/user/?id=" + data.id + "&isLocked=" + !data.accountNonLocked
      ).then(resp => {
        if (resp) {
          this.initUserData();
        }
